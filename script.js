...
const store = {
	...
	state: {
		...
		config: {
			...
			hideControls: true, // Always hide controls
			longExposure: false,
...
function renderApp(state) {
	...
	// Safely update elements that may not exist (controls removed)
...
const heartShell = (size = 1) => {
	...
	return {
		...
		glitter: 'light',
		glitterColor: Math.random() < 0.5 ? COLOR.White : color,
...
function init() {
	// Remove loading state (safely check if element exists)
	...
	// Don't begin simulation here - wait for intro to complete
	// togglePause(false); // Moved to startFireworks()
	...
	burst(x, y) {
		...
		if (typeof this.color === 'string') {
			...
			// Heart shape burst
				...
				const heartPoints = Math.ceil(this.starCount * 1.5);
				const heartScale = this.spreadSize / 120;

				// 30% chance to rotate the heart by ±45 degrees
				const shouldRotate = Math.random() < 0.3;
				const rotationAngle = shouldRotate ? (Math.random() * 0.5 - 0.25) * Math.PI : 0; // ±45 degrees in radians

				for (let i = 0; i < heartPoints; i++) {
					const t = (i / heartPoints) * PI_2;
					// Parametric heart curve equations
					const heartX = 16 * Math.pow(Math.sin(t), 3);
					// Flip Y to make heart point upward (positive Y = down in screen coords)
					const heartY = (13 * Math.cos(t) - 5 * Math.cos(2 * t) - 2 * Math.cos(3 * t) - Math.cos(4 * t));

					// Apply rotation if needed
					let targetX = heartX * heartScale;
					let targetY = heartY * heartScale;
					...
					const angle = MyMath.pointAngle(0, 0, targetX, targetY);
					const velocity = MyMath.pointDist(0, 0, targetX, targetY) / 100;
...
// ============================================================
// INTRO SYSTEM - Text messages before fireworks
// ============================================================

// CONFIGURE YOUR MESSAGES HERE - Add or modify messages in this array
const introMessages = [
	"Xin chào em bé Cát Nhi xinh đẹp của anh",
	"Cảm ơn em bé vì một năm qua đã luôn ở bên anh",
	"Yêu thương anh",
	"Cùng anh vượt qua thật nhiều vui buồn cùng nhau",
	"Anh mong là mình cũng sẽ đồng hành cùng nhau lâu dài hơn nữa",
	"Cảm ơn em bé vì một năm 2025 thật tuyệt vời",
	"Chúc em bé của anh năm mới thật khỏe mạnh, học hành giỏi giang và vui vẻ hơn",
	"Chúc cho chúng ta của 2026 thật hạnh phúc với nhau",
	"Happy New Year mah Babyboo 🥰",
	"Em bé ấn vào màn hình thật nhiều để năm mới bé cũng có thật nhiều may mắn nhé 😎"
];

// Time between each message (in milliseconds)
const messageDisplayTime = 3000; // 2 seconds per message
const introAutoStartTime = 35000; // 33 seconds total before auto-start
...
function showIntroMessage(index) {
	...
	console.log('showIntroMessage called with index:', index);

	if (!introOverlay || !introText) {
		console.log('ERROR: intro elements not found');
		return;
	}

	if (introCompleted) {
		console.log('Intro already completed, skipping');
		return;
	}
	...
	// If we've shown all messages, mark it and wait for tap
	if (index >= introMessages.length) {
		allMessagesShown = true;
		index = introMessages.length - 1;
	}

	currentMessageIndex = index;
	const isLastMessage = index === introMessages.length - 1;
	const isFirstMessage = index === 0;

	// For first message, show immediately without fade
	if (isFirstMessage) {
		introText.textContent = introMessages[index];
		introText.classList.add('visible');
		console.log('First message shown:', introMessages[index]);

		// Schedule next message
		if (!isLastMessage) {
			messageTimer = setTimeout(() => {
				showIntroMessage(currentMessageIndex + 1);
			}, messageDisplayTime);
		} else {
			allMessagesShown = true;
		}
	...
	// For subsequent messages, fade out then fade in
	...
	setTimeout(() => {
		...
		// Add tap-message class for last message (pulsing animation)
		if (isLastMessage) {
			introText.classList.add('tap-message');
		} else {
			introText.classList.remove('tap-message');
		}

		// Fade in new text
		introText.classList.add('visible');
		console.log('Message shown:', introMessages[index]);

		// If not last message, schedule next message (auto-advance)
		if (!isLastMessage) {
			messageTimer = setTimeout(() => {
				showIntroMessage(currentMessageIndex + 1);
			}, messageDisplayTime);
		} else {
			// Mark all messages shown when we reach the last one
			allMessagesShown = true;
		}
	}, 400);
}

// Handle tap on intro overlay
function handleIntroTap(e) {
	if (e) e.preventDefault();

	if (introCompleted) return;

	// If all messages shown, start fireworks
	if (allMessagesShown) {
		startFireworks();
	} else {
...
function startFireworks() {
	...
	// Start simulation now (was paused during intro)
	togglePause(false);

	// Resume audio (required for browser AudioContext after user interaction)
	soundManager.resumeAll();

	// Enable auto-launch and start fireworks
	...
	// Launch a few shells immediately for wow effect
	setTimeout(() => {
	...
	// Start finale mode toggle every 30 seconds
	startFinaleModeToggle();
}

// Toggle finale mode on/off every 30 seconds
function startFinaleModeToggle() {
	setInterval(() => {
		const currentFinale = store.state.config.finale;
		updateConfig({ finale: !currentFinale });
	}, 30000); // 30 seconds
}

function setupIntro() {
	console.log('setupIntro called');
	const introOverlay = document.getElementById('intro-overlay');
	console.log('intro-overlay element:', introOverlay);

	if (introOverlay) {
		console.log('intro-overlay found, calling showIntroMessage(0)');
		// Start showing messages
		showIntroMessage(0);

		// Auto-start fireworks after 20 seconds if user doesn't tap
		introAutoStartTimer = setTimeout(() => {
			startFireworks();
		}, introAutoStartTime);

		// Listen for tap/click - skip message or start fireworks
		introOverlay.addEventListener('click', handleIntroTap);
		introOverlay.addEventListener('touchstart', handleIntroTap, { passive: false });
	} else {
		console.log('ERROR: intro-overlay NOT found!');
	}
}
...
function initApp() {
	console.log('initApp called - starting intro');
	init();
	setupIntro();
}
